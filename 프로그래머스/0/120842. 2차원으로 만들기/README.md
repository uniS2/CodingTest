...
- `for` 문을 사용하여 반복 조건을 작성하였습니다.
- `slice` 메서드를 사용하여 증가하는 i 값에 따른 로직을 작성하였습니다.

<br>

#### 🆕 `splice` 메서드를 이용한 다른 풀이
...
- `while` 문으로 간단하게 반복 조건을 작성하였습니다.
- `splice` 메서드의 원본배열을 변경하는 특성을 이용하여 로직을 작성하였습니다.
- 기존 풀이보다 간단해졌음을 알 수 있습니다.

✨ 결론

- `for` 문이 복잡할 경우 `while` 문을 이용한 풀이를 고려하자.
- `slice` 메서드 보다 `splice` 메서드가 간단한 경우가 있다. 고려해보자 🤓
